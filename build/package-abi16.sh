#!/usr/bin/env bash
# package.sh - packages the finished module (after build-abi16.sh) into awg-quick-magisk-abi16-17.zip
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MODULE_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
DIST_DIR="$SCRIPT_DIR/dist"
ZIP_NAME="awg-quick-magisk-abi16-17.zip"

mkdir -p "$DIST_DIR"
rm -f "$DIST_DIR/$ZIP_NAME"

if [ ! -d "$MODULE_DIR/bin/arch" ] || [ -z "$(ls -A "$MODULE_DIR/bin/arch" 2>/dev/null)" ]; then
  echo "ERROR: bin/arch is empty. Run build-abi16.sh first to build the binaries." >&2
  exit 1
fi
